...
public class UserDAO {
    ...
    public boolean createUser(User user) throws SQLException {
        try (Connection conn = DBUtil.getConnection();
            ...
            stmt.setString(4, user.getAddress() != null ? user.getAddress().trim() : null);
            stmt.setString(5, user.getContact() != null ? user.getContact().trim() : null);
    ...
    public boolean updateUser(User user) {
        String sql = "UPDATE User SET name = ?, email = ?, contact = ?, address = ?, picture = ?, role = ? WHERE User_ID = ?";
        try (Connection conn = DBUtil.getConnection();
